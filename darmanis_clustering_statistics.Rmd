...
tsne <- lapply(normalized.data, function(dataset) {
                        tsne.res <- Rtsne::Rtsne(t(dataset[,-1]),
...
cell type info. 
```{r Function for plotting tsne}
# Make a function for plotting tsne's and using 
TsnePlot <- function(dat, var, name = "name"){
...
"nmi"and Adjusted Rand Index "ari" statistics over iterations (default is 10). 
...
batch.results <- lapply(tsne, function(dataset) {
                        # Get KNN accuracy results
                        kmeans.results <- KmeansEval(dataset, metadata = plate.batch)
...
PlotResults(cell.type.results, name = "Cell_type")

# Plot the plate batch results
PlotResults(batch.results, name = "Plate_batch")
```
