...
all.data <- all.data %>% dplyr::select(-dplyr::contains("10X_P4_2"))
```
...
# Double check that metadata and data match
all.equal(metadata$full.id, colnames(all.data))
```
Let's look at total counts for these samples

```{r}
# Calculate the total number of counts for each sample
total_counts <- apply(all.data, 2, sum)

# Histogram of this data
hist(total_counts, breaks = 50, col = "lightblue"); 
...
Filter out samples based on total counts.

```{r}
# Filter out these samples from data and metadata
filtered_data <- all.data[, which(total_counts > 2000)]
filtered_metadata <- metadata[which(total_counts > 2000), ]

# Double check everything matches
all.equal(colnames(filtered_data), filtered_metadata$full.id)
```

Make a data matrix for ease of collecting stats

```{r}
data.mat <- filtered_data %>% as.matrix()
```
...
sample.sum.df <- data.frame(sample.sum, filtered_metadata) 
...
filtered_data <- filtered_data[, which(sample.sum > 300)] %>% 
  dplyr::filter(gene.sum > 10)

# Add the genes column to the front
filtered_data <- data.frame("genes" = gene.names[which(gene.sum > 10)],
                            filtered_data)
...
saveRDS(filtered_data, file.path("tab_mur_data", "filtered_counts_tab_mur.RDS"))
```
