...
dataset <- readr::read_tsv(file.path("darmanis_data", "unfiltered_darmanis_counts.tsv"), 
...
# Optional save point after ids are recoded
# readr::write_tsv(dataset, file.path("darmanis_data", "unfiltered_darmanis_counts.tsv"))

# Filter out samples not in the dataset
geo.meta <- geo.meta[match(colnames(dataset)[-1], geo.meta$geo_accession), ]
```

Make percent mapped reads data into geo_accession ids.

```{r Set up percent_mapped}
# Change this data to geo accession ids as well
readr::read_tsv(file.path("darmanis_data",
                          "darmanis_data_percent_mapped_reads.tsv")) %>% 
  dplyr::mutate("geo_accession" = dplyr::recode(geo_accession, !!!convert.key)) %>%
  readr::write_tsv(file.path("darmanis_data", "darmanis_data_percent_mapped_reads.tsv"))
```
...
```{r Filter out genes}
prepped.data <- GeneMatrixFilter(dataset, min_counts = 1, num_genes = 3,
                                 num_genes = 100)
```
...
# Optional unfiltered metadata save to tsv file
# geo.meta %>% readr::write_tsv(file.path("darmanis_data", 
