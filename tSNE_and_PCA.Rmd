...
geo.meta <- geo.meta[-grep("characteristics", names(geo.meta))]

# Get rid of the ch1 tag
names(results) <- gsub("\\.ch1$", "", names(results))
names(geo.meta) <- names(results)
```
...
pc.plot <- function(dat, var, label = "label"){
  # Coerce it to a factor in case it's not. 
  var <- as.factor(var)
...
lapply(geo.meta, function(variable) {
  ...
  # Make PCA plot
  pc.plot(pca$x, var = variable, label = paste0(name, "_pca"))
  
  # Make tSNE plot
  pc.plot(tsne.dat, var = variable, label = paste0(name, "_tsne"))
})
```
