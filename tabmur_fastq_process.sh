...
#_-------------------Convert bam files to fastq files -------------------------#
cd tab_mur_bam
...
for folder in `ls`
  ...
  for f in `ls *_R1_001.fastq.gz | sed 's/_R1_001.fastq.gz//' `
    ...
    salmon alevin -l ISR  \
      --no-version-check \
      -i ../../../ref_files/human_index \
      -1 ${f}_R1_001.fastq.gz ${f}_R1_002.fastq.gz ${f}_R1_003.fastq.gz \
      -2 ${f}_R2_001.fastq.gz ${f}_R2_002.fastq.gz ${f}_R2_003.fastq.gz \
      --chromium  \
      -p 10 \
      -o ../../alevin_output/${f} \
